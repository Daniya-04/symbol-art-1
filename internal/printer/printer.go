...
func Render(input string, b banner.Banner) []string {
	...
	// Вход из одних только "\n" (без единого другого символа) — тоже
	// особый случай. strings.Split даёт N+1 сегментов на N разделителей,
	// то есть на "\n" (N=1) получилось бы 2 пустых сегмента и 2 пустые
	// строки вывода — а ожидается ровно N: один "\n" даёт одну пустую
	// строку. Правило "N+1 сегментов" ниже остаётся верным для любого
	// текста с реальным содержимым (там лишний хвостовой пустой сегмент
	// как раз и означает "перевод строки после текста"), но здесь текста
	// нет вообще, и добавлять его не должно.
	if strings.Trim(input, "\n") == "" {
		return make([]string, strings.Count(input, "\n"))
	}
